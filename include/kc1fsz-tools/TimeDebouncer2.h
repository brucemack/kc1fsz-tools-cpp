 ...
 */
class TimeDebouncer2 : public BinaryWrapper {
public:

    TimeDebouncer2(Clock& clock, std::function<bool()> rawValue)
    :   _clock(clock),
        _rawValue(rawValue),
        _debouncedState(rawValue),
    ...
    bool get() const {
        ...
        if (_debouncedState == rawState) {
           const_cast<TimeDebouncer2*>(this)->_lastStableTime = _clock.time();
        }
        ...
        // the time and see if we make it through the transition window.
            ...
            if ((_clock.time() - _lastStableTime) > transitionTimeMs) {
                const_cast<TimeDebouncer2*>(this)->_debouncedState = !_debouncedState;
            }
        }
...
private:
    ...
    const Clock& _clock;
    std::function<bool()> _rawValue = nullptr;
